...
const getAllGroups = async (req, res) => {
  ...
  const group = await client.query(
    `select "studentsGroup".students_id, "group".id, "group".name, Teacher.first_name, Teacher.last_name, (select count(students_id) as students_count from public."studentsGroup" where group_id = "group".id)  
...
UNION
select Teacher.id, "group".id, "group".name, Teacher.first_name, Teacher.last_name, (select count(students_id) as students_count from public."studentsGroup" where group_id = "group".id)  
...
const getGroup = async (req, res) => {
  ...
  const group = await client.query(
    `select "studentsGroup".students_id, Student.first_name, Student.last_name 
    from public."studentsGroup"
    INNER JOIN public."user" as Student ON "studentsGroup".students_id = Student.id 
