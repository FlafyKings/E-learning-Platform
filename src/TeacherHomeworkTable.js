                        ...
                        <TableCell align="right">
                          <Tooltip title="Oceń test">
                            <IconButton
                              size="small"
                              onClick={() =>
                                navigate(`/homework/grade/${row.answerid}`)
                              }
                            >
                              <EditIcon
  ...
  useEffect(() => {
    ...
    const getTest = async () => {
      try {
        ...
        isMounted && setTests(response.data.homeworks);
        setUngraded(response.data.ungradedHomework);
        setAttempts(response.data.allAttemptsHomework);

        setTests(
          response.data.homeworks.rows.map((row) => {
            return createDataTests(row);
          })
        );

        setUngraded(
          response.data.ungradedHomework.rows.map((row) => {
            return createData(row);
          })
        );

        setAttempts(
          response.data.allAttemptsHomework.rows.map((row) => {
            return createData(row);
          })
        );
      } catch (err) {
